...
class SaveBobSessionRecordUseCase : IUseCase<Unit, Boolean> {
    ...
    override suspend fun invoke(params: Map<String, String>, bean: Unit): Result<Boolean> {
        return runCatching {
            ...
            val x3DHKeyStr = params["bobX3DHKey"]
                ?: return Result.failure(Exception("x3DHKey is null"))
            val aliceIdentityKey = params["aliceIdentityKey"]
                ?: return Result.failure(Exception("aliceIdentityKey is null"))
            val aliceEphemeralKey = params["aliceEphemeralKey"]
                ?: return Result.failure(Exception("aliceEphemeralKey is null"))
            val bobIdentityKey = identityKeyDao.getByUserId(localUserId)
                ?: return Result.failure(Exception("bobIdentityKey is null"))
            val bobSignedPreKey = signedPreKeyDao.getByUserId(localUserId)
